...
import django.contrib.postgres.fields as psqlFields
class Philly_request(models.Model):
    _DATABASE = "psql"
    user = models.CharField(max_length = 20)
    taskname = models.CharField(max_length = 20)
    taskid = models.IntegerField()
    parameter = psqlFields.JSONField()
    finish = models.IntegerField()

class Philly_congestion(models.Model):
    _DATABASE = "psql"
    request = models.ForeignKey(Philly_request, on_delete=models.CASCADE)
    link = models.ForeignKey(Philly_link)
    time = models.DateTimeField()
    value = models.FloatField()

class Philly_congestion_array(models.Model):
    _DATABASE = "psql"
    request = models.ForeignKey(Philly_request, on_delete=models.CASCADE)
    link = models.ForeignKey(Philly_link)
    values = psqlFields.ArrayField(models.FloatField())

class Philly_DMS(models.Model):
    _DATABASE = "psql"
    no = models.IntegerField()
...
class Philly_online_loading(models.Model):
    _DATABASE = "psql"
    time = models.DateTimeField()


class Philly_congestion_array_online(models.Model):
    _DATABASE = "psql"
    loading = models.ForeignKey(Philly_online_loading, on_delete=models.CASCADE)
    link = models.ForeignKey(Philly_link)
    values = psqlFields.ArrayField(models.FloatField())
...
class Permissions(models.Model):
    class Meta:
        permissions = (
            ...
            ('perm_congestion', 'permission to access congestion section'),
        )
