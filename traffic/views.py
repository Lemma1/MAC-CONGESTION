...
def get_RT_incidents_twitter(request):
    ...
    sensi  = request.GET['sensi']
    print 'sensitivity: ',sensi
    rawTweets = pickle.load(open(fileName,"rb"))
    IUID2IUName = pickle.load(open('/tweetSource/IUID2IUName.p','rb'))
    ...
    tweetsTemp.sort(key = lambda x: x['timestamp'])
    jsonJJList = []
    causes={}
    newTweetTemp = []
    for atweet in tweetsTemp:
        if atweet['category'] != "Not traffic incident":
            if atweet['location'] is not None:
                ...
                if ('probTI' not in atweet) or (('probTI' in atweet) and (valuesensi(atweet['probTI'],sensi))):
                    if 'probTI' not in atweet:
                        atweet['probTI'] = -1
                        atweet['probCate'] = -1
                    ...
                    jj = {}
                    jj["type"]="Feature"
                    atweet['text']=re.sub('http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+', '', atweet['text'])

                    # IUID2IUName[0] = "WazeTrafficPITT"
                    if 'uid' not in atweet:
                        atweet['uid']   =  0
                    if 'uname' not in atweet:
                        atweet['uname'] = 'X'




                    jj["properties"]={"eventid":atweet['tid'],"direction":"NO","start":str(atweet['timestamp']),"end":str(atweet['timestamp']),"duration":"00:00:00","delay":"0","cause":atweet['category'],"sr":"0000","text":atweet['text'],"probTI":atweet['probTI'],"probCate":atweet['probCate'],"uid":atweet['uid'],"uname":atweet['uname']}
...
@permission_required(perm= 'traffic.perm_twitter_map', raise_exception= False)
def update_incidents_twitter(request):
    fileName = '/tweetSource/twitterDBDic.p'
    rawTweets = pickle.load(open(fileName,"rb"))
    cate = request.GET['submit_cate']
    tid = request.GET['ttid']
    print "CATE: ",cate
    print "TID:  ",tid
    for tweet in rawTweets:
        # print tweet['tid']
        if tweet['tid']/1000==int(tid)/1000:
            print 'GETONE!!!!!!!!!'
            print tweet['category']
            tweet['category'] = cate
            print tweet['category']
            filename2 = 'updatetweet-'+str(tweet['tid'])+'.p'
            pickle.dump(tweet,open(filename2,'wb'))
            # pickle.dump(rawTweets,open(fileName,'wb'))

    return HttpResponse('{1:1}', content_type='application/json')
...
def get_incidents_twitter(request):
    ...
    e_hour = int(e_time[0:2])
    e_minute = int(e_time[2:4])
    start_date = date(int(s_date[0:4]),int(s_date[4:6]), int(s_date[-2:]))
    end_date = date(int(e_date[0:4]),int(e_date[4:6]), int(e_date[-2:]))
    ...
    start_datetime = datetime(int(s_date[0:4]),int(s_date[4:6]),int(s_date[-2:]),s_hour,s_minute,1)
    end_datetime   = datetime(int(e_date[0:4]),int(e_date[4:6]),int(e_date[-2:]),e_hour,e_minute,1)
    print start_datetime
    print end_datetime
    print "sensitivigy: ",sensi
    ...
    for atweet in tweetsTemp:
        if atweet['category'] != "Not traffic incident":
            if atweet['location'] is not None:
                # print 'probTI, sensi', atweet['probTI'],sensi
                # print 'result: ',valuesensi(atweet['probTI'],sensi)
                if ('probTI' not in atweet) or (('probTI' in atweet) and (valuesensi(atweet['probTI'],sensi))):
                    if 'probTI' not in atweet:
                        atweet['probTI'] = -1
                        atweet['probCate'] = -1
...
def congestion_task_request(request):
    return render(request, 'traffic/congestion_task_request.html')

# @permission_required(perm= 'traffic.congestion', raise_exception= True)
def congestion_task_view(request):
    return render(request, 'traffic/congestion_task_view.html')
...
def get_links(request):
    ...
    numlanes = int(request.GET['numlanes'])
    ...
    if numlanes != 0:
        links = Philly_link.objects.filter(geom__bboverlaps = boxDiagnol).filter(zoom_level__exact = numlanes)
    else:
        links = Philly_link.objects.filter(geom__bboverlaps = boxDiagnol).filter(zoom_level__gte = numlanes)
    # links = links.prefetch_related('geom')

    current_milli_time = lambda: int(round(time.time() * 1000))
    starting_milli = current_milli_time()
    track = lambda: current_milli_time() - starting_milli

    response = serializers.serialize('json', links, fields=('geom_str'))
